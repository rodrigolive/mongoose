...
sub find {
    ...
    if ( scalar @_ && ref( $_[ 0 ] ) ne 'HASH' ) { $query = { @_ }; }
    else                                         { ( $query, $attributes ) = @_; }
    #use Data::Dumper;
...
sub update_or_create {
    ...
    if ( $maybe and my $match = $maybe->first ) {
        ...
        $match->resultset->update( $modification );
...
sub remove {
    my ( $self, $options ) = @_;
    return $self->_class->collection->remove( $self->_query,
        { %{ $self->_attributes ? $self->_attributes : {} }, %{ $options ? $options : {} } } );
}
...
sub new_result {
    my $self = shift;
    my %values;
    if   ( scalar @_ && ref( $_[ 0 ] ) eq 'HASH' ) { %values = %{ $_[ 0 ] }; }
    else                                           { %values = @_; }
...
sub _clone {
    my $self = shift;
    return ( ref $self )
        ->new( _class => $self->_class, _query => $self->_query, _attributes => $self->_attributes, _fields => $self->_fields );
}
...
sub _collapse_hash {
    ...
    for my $attribute ( keys %{ $set } ) {
        if (    defined $set->{ $attribute }
            and $self->_class->meta->has_attribute( $attribute )
            and $self->_class->meta->get_attribute( $attribute )->type_constraint->name =~ m{(Int|Num)}
            and not defined reftype $set->{ $attribute } )
        {
...
# rodrigo: outsider Document methods

sub insert {
    my $self = shift;
    return $self if $self->in_storage;
    $self->save;
    $self = $self->find_one({ _id => $self->_id });
    return $self;
}
