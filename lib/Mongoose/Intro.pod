...
one of two roles: Mongoose::Document or Mongoose::EmbeddedDocument.
...
but a more object oriented notation is also supported.
	...
	use Mongoose;
	Mongoose->db( db=>'mydb'); # looks for a localhost connection
...
Tells Mongoose to store this as an embedded document, part of 
a parent document. 
